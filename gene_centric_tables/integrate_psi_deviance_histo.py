...
infile_pat = ['merge_graphs_*_C%i.psi_dev_histo_per_gene_func.tsv.gz' % CONF, 'merge_graphs_*_C%i.psi_dev_histo_per_gene.tsv.gz' % CONF]
outfile_pat = ['merge_graphs_all_events_C%i.psi_dev_histo_per_gene_func.tsv.gz' % CONF, 'merge_graphs_all_events_C%i.psi_dev_histo_per_gene.tsv.gz' % CONF]
outfile_pat2 = ['merge_graphs_all_events_C%i.psi_dev_histo_per_gene_binary_func.tsv.gz' % CONF, 'merge_graphs_all_events_C%i.psi_dev_histo_per_gene_binary.tsv.gz' % CONF]
outfile_pat3 = ['merge_graphs_all_events_C%i.psi_dev_median_histo_per_gene_func.tsv.gz' % CONF, 'merge_graphs_all_events_C%i.psi_dev_median_histo_per_gene.tsv.gz' % CONF]
functional_tag = ['_func', '']

for p in [1]: #range(len(infile_pat)):
    ...
    if 'plot_only' in sys.argv:
        ...
        for f, fname in enumerate(filelist):
            ...
            if f == 0:
                ...
                for i,c in enumerate(cnt):
                    if c > 1:
                        newmerged[i, :] = merged[cumcounts:cumcounts+c, :].max(axis=0)
                        tmp = merged[cumcounts:cumcounts+c, :].argmax(axis=0)
        ...
        s_idx = sp.argsort(sp.median(merged, axis=1))[::-1]
        merged = merged[s_idx, :]
        merged_ids = merged_ids[s_idx, :]
        genes = genes[s_idx]
   ...
   # projects = metadata[b, pidx]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plotdir, 'normalized_psi_pca_histo_integrated_C%i%s.png' % (CONF, functional_tag[p])), format='png', bbox_inches='tight')
    plt.close(fig)
