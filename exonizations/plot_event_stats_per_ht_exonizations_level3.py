...
def main():
    ...
    for e, event_type in enumerate(event_types):
        ...
        ### is exonization 
        is_exonization = pickle.load(open(os.path.join(BASEDIR_ICGC, 'merge_graphs_%s_C%i.exonize_candidates_step3.pickle' % (event_type, CONF)), 'r'))
    ...
    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level3_C%i_%s.pdf' % (CONF, p)), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level3_C%i_%s.png' % (CONF, p)), format='png', bbox_inches='tight')
        plt.close(figs[p])
        sp.savetxt(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level3_C%i_%s.tsv' % (CONF, p)), source_data[p], fmt='%s', delimiter='\t')
