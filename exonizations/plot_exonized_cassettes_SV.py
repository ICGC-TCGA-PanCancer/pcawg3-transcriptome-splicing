...
TOP = 274
CONF = 2
PLOTDIR = os.path.join(BASEDIR_AS, 'alternative_splicing/plots/exonization_detail_SV_C%s' % CONF)
...
for i in range(min(TOP, results.shape[0])):
    ...
    print('Processing %s %s' % (results[i, 0], results[i, 1]), file=sys.stderr)

    alt_donors = sp.array(results[i, 12].split(','))
    ref_donors = sp.array(results[i, 13].split(','))
    projects = results[i, 14].split(',')
    files_alt = results[i, 15].split(',')
    files_ref = results[i, 16].split(',')

    var_pos = [int(x.split(':')[1]) for x in results[i, 0].split(',')]

    ### generate intron filter
    event_pos = sp.array([int(results[i, 3]) - 100, int(results[i, 8]) + 100])
    exons2 = sp.reshape(results[i, 3:9], (3, 2)).astype('int')
    exons1 = exons2[[0, 2], :]
    if (exons2[1, 1] - exons2[1, 0]) % 3 == 0:
        in_frame = 'yes'
    else:
        in_frame = 'no'
    intron_filter = [(int(results[i, 4]), int(results[i, 5])),\
                     (int(results[i, 6]), int(results[i, 7])),\
                     (int(results[i, 4]), int(results[i, 7]))]
    ...
    for d, donor in enumerate(alt_donors):
        ...
        ylim = ax.get_ylim()
        xlim = ax.get_xlim()
        xlim = [max(xlim[0], event_pos.min() - 50), min(xlim[1], event_pos.max() + 50)]
        xspan = xlim[1] - xlim[0]
        yspan = ylim[1] - ylim[0]
        for vp in var_pos:
    ...
    #if un.get_ID(results[i, 9], lookup=lookup) == 'STK11':
    #    import pdb
    #    pdb.set_trace()
    ### plot event layout
