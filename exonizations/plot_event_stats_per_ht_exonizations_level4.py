...
event_dict = {'exon_skip':'Novel Exonizations',
...
def main():
    figs = dict()
    figs['stats'] = plt.figure(figsize=(15, 6))
    figs['stats_rel'] = plt.figure(figsize=(15, 6))
    figs['stats_rel_sampsize'] = plt.figure(figsize=(15, 6))
    gss = dict()
    ...
    for e, event_type in enumerate(event_types):
        ...
        ### is exonization 
        is_exonization = pickle.load(open(os.path.join(BASEDIR_ICGC, 'merge_graphs_%s_C%i.exonize_candidates_step4.pickle' % (event_type, CONF)), 'r'))
        ...
        ax.bar(sp.arange(htypes.shape[0]) + 0.5, counts_anno, 0.6, color=colors, linewidth=0.5, edgecolor='black')
        ...
        ax.yaxis.grid(True)
        source_data['stats'] = sp.c_[htypes[:, sp.newaxis], counts_anno[:, sp.newaxis].astype('str')]

        ### plot stats for events by histotype - relative to events detected
        ax = figs['stats_rel'].add_subplot(gss['stats_rel'][e / 3, e % 3])
        ax.bar(sp.arange(htypes.shape[0]) + 0.5, counts_anno / counts_all.astype('float'), 0.6, color=colors, linewidth=0.5, edgecolor='black')
        axs.set_ticks_outer(ax)
        axs.clean_axis(ax)
        ax.set_xticks(sp.arange(htypes.shape[0]) + 0.5)
        ax.set_xlim([-0.2, htypes.shape[0]])
        ...
        ax.set_title(event_dict[event_type])
        ax.yaxis.grid(True)
        source_data['stats_rel'] = sp.c_[htypes[:, sp.newaxis], (counts_anno / counts_all.astype('float'))[:, sp.newaxis].astype('str')]
        ...
        ax.bar(sp.arange(htypes.shape[0]) + 0.5, counts_anno * counts_sf, 0.6, color=colors, linewidth=0.5, edgecolor='black')
        axs.set_ticks_outer(ax)
        axs.clean_axis(ax)
        ax.set_xticks(sp.arange(htypes.shape[0]) + 0.5)
        ax.set_xlim([-0.2, htypes.shape[0]])
        ...
        ax.set_title(event_dict[event_type])
        ax.yaxis.grid(True)
        source_data['stats_rel_sampsize'] = sp.c_[htypes[:, sp.newaxis], (counts_anno * counts_sf)[:, sp.newaxis].astype('str')]


    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level4_C%i_%s.pdf' % (CONF, p)), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level4_C%i_%s.png' % (CONF, p)), format='png', bbox_inches='tight')
        plt.close(figs[p])
        sp.savetxt(os.path.join(PLOTDIR, 'event_overview_per_ht_exonize_level4_C%i_%s.tsv' % (CONF, p)), source_data[p], fmt='%s', delimiter='\t')
