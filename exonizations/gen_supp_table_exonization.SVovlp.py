...
fin_data = sp.array(fin_data).T
kidx = sp.where(fin_data[:, -1] != 'NA')[0]
fin_data = fin_data[kidx, :]

### gen header
header = sp.array(['event_id', 'event_pos', 'strand', 'ensemble_id', 'gene_name', 'max_dpsi', 'coding_status', 'overlap_snv'])
fin_data = sp.r_[header[sp.newaxis, :], fin_data]
sp.savetxt(os.path.join(basedir, 'supplemental_table_exonization_candidates.SVovlp.tsv'), fin_data, fmt='%s', delimiter='\t') 
