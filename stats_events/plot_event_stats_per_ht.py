...
def main():
    ...
    for e, event_type in enumerate(event_types):
        ...
        idx_psi_icgc = pickle.load(open(os.path.join(BASEDIR_ICGC, 'merge_graphs_%s_C%i.counts.hdf5.psi_filt_per_ht.pickle' % (event_type, CONF)), 'r'))[1]
    ...
    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_C%i_%s.pdf' % (CONF, p)), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_per_ht_C%i_%s.png' % (CONF, p)), format='png', bbox_inches='tight')
        plt.close(figs[p])
