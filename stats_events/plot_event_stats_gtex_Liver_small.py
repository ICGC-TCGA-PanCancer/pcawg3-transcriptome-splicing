...
BASEDIR = os.path.join(BASEDIR_AS, 'alternative_splicing_gtex_Liver_small')
...
def main():
    ...
    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_gtex_%s_Liver_small.pdf' % p), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_gtex_%s_Liver_small.png' % p), format='png', bbox_inches='tight')
        plt.close(figs[p])
