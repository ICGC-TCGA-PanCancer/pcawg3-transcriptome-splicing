...
BASEDIR      = os.path.join(BASEDIR_AS, 'alternative_splicing_LIRI-JP_small_tumor')
...
def main():
    ...
    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_%s_LIRI-JP_small_tumor.pdf' % p), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_%s_LIRI-JP_small_tumor.png' % p), format='png', bbox_inches='tight')
        plt.close(figs[p])
