...
BASEDIR      = os.path.join(BASEDIR_AS, 'alternative_splicing_LIRI-JP_small_normal')
...
def main():
    ...
    for e, event_type in enumerate(event_types):
        ...
        anno = pickle.load(open(os.path.join(BASEDIR_ANNO, 'merge_graphs_%s_C3.pickle' % event_type), 'r'))[0]

        ### load annotation index
        is_anno = pickle.load(open(os.path.join(BASEDIR, 'merge_graphs_%s_C3.anno_only.pickle' % event_type), 'r'))

        ### load confident events
        IN = h5py.File(os.path.join(BASEDIR, 'merge_graphs_%s_C3.counts.hdf5' % event_type), 'r')
        idx_conf = IN['conf_idx'][:]
        IN.close()

        ### load filtered events
        IN = h5py.File(os.path.join(BASEDIR, 'merge_graphs_%s_C3.counts.r10_s25_V10.hdf5' % event_type), 'r')
    ...
    for p in figs:
        figs[p].tight_layout()
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_%s_LIRI-JP_small_normal.pdf' % p), format='pdf', bbox_inches='tight')
        figs[p].savefig(os.path.join(PLOTDIR, 'event_overview_%s_LIRI-JP_small_normal.png' % p), format='png', bbox_inches='tight')
        plt.close(figs[p])
