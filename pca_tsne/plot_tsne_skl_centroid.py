...
for event_type in event_types[:1]:
    ...
    picklefile = '%s/tsne_skl_%s_centroid.TN.conf_%.2f.perp_%i%s.pickle' % (datadir, event_type, 1.0 - conf, perplexity, filter_tag)
    ...
    if not os.path.exists(picklefile):
        ...
        if do_filter:
            ...
            ### filter to not highly complex genes
        ...
        ### determine coefficient of variation per project code + tumor normal (ICGC) / tissue type (GTEx)
        ctypes = sp.r_[sp.array([ct_dict[x] for x in strains]), sp.array([gt_dict[x] for x in strains_gt])]
        for c in range(strains.shape[0]):
            if is_tumor_dict[strains[c]]:
                ctypes[c] += '-T'
            else:
                ctypes[c] += '-N'
        ctypes_u = sp.unique(ctypes) 
        cv = sp.zeros((psi.shape[0], ctypes_u.shape[0]), dtype='float')
        for c, ct in enumerate(ctypes_u):
            ct_idx = sp.where(ctypes == ct)[0]
            cv[:, c] = sp.std(psi[:, ct_idx], axis=1) / sp.mean(psi[:, ct_idx], axis=1)
        nn_idx = sp.where(~sp.isnan(sp.mean(cv, axis=1)))[0]
        s_idx = sp.argsort(sp.mean(cv, axis=1)[nn_idx])[::-1]
        nn_idx = nn_idx[s_idx][:1000]
        
        psi = psi[nn_idx, :]

        ### compute median psi per study / tissue
        psim = sp.zeros((psi.shape[0], ctypes_u.shape[0]), dtype='float')
        for c, ct in enumerate(ctypes_u):
            ct_idx = sp.where(ctypes == ct)[0]
            psim[:, c] = sp.mean(psi[:, ct_idx], axis=1)

        psi = sp.array([[spst.pearsonr(psim[:, c1], psim[:, c2])[0] for c2 in range(psim.shape[1])] for c1 in range(psim.shape[1])])
        #psi = sp.cov(psi)
        ...
        #cPickle.dump((trans_data, strains, strains_gt), open(picklefile, 'w'), -1)
        pickle.dump((trans_data, ctypes_u), open(picklefile, 'w'), -1)
        ctypes = ctypes_u
    else:
        print('Loading data from pickle: %s' % picklefile)
        #(trans_data, strains, strains_gt) = cPickle.load(open(picklefile, 'r'))
        (trans_data, ctypes) = pickle.load(open(picklefile, 'r'))
    ...
    #nn_idx = (htypes!='NA')
    #htype_colors = dict(zip(sp.unique(htypes[nn_idx]), ic.get_color_scheme('tumor_subtype', labels=sp.unique(htypes[nn_idx]))))
    #ctypes_gt = sp.array([gt_dict[x] for x in strains_gt])
    ...
    for k1 in range(0, k):
        ...
        for k2 in range(k1 + 1, k):
            ...
            for idx, ct in enumerate(sp.unique(ctypes)):
                ...
                if c_idx.shape[0] > 0:
                    ax.plot(trans_data[k1, c_idx], trans_data[k2, c_idx], color_dict[ct][1], color = cmap(norm(color_dict[ct][0])), label = ct, ms=15, alpha=0.75)
                    #ax.plot(trans_data[k1, c_idx], trans_data[k2, c_idx], markers.MarkerStyle.filled_markers[idx % 13], color = cmap(norm(idx)), label = ct, ms=20, alpha=0.75)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.CT.%i.conf_%.2f.perp_%i%s.png' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'png', bbox_inches='tight')
    plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.CT.%i.conf_%.2f.perp_%i%s.pdf' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'pdf', bbox_inches='tight')
    plt.close(fig)
    print('... done.')
    ...
    #plt.tight_layout()
    #plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.HT.%i.conf_%.2f.perp_%i%s.png' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'png', bbox_inches='tight')
    #plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.HT.%i.conf_%.2f.perp_%i%s.pdf' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'pdf', bbox_inches='tight')
    #plt.close(fig)
    #print '... done.'
    ...
    #plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.TN.%i.conf_%.2f.perp_%i%s.png' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'png', bbox_inches='tight')
    #plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.TN.%i.conf_%.2f.perp_%i%s.pdf' % (event_type, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'pdf', bbox_inches='tight')
    #print '... done.'
    #plt.close(fig)
    ...
    #    plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.%s.%i.conf_%.2f.perp_%i%s.png' % (event_type, s, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'png', bbox_inches='tight')
    #    plt.savefig(os.path.join(plotdir, 'tsne_skl_%s_centroid.%s.%i.conf_%.2f.perp_%i%s.pdf' % (event_type, s, k, 1.0 - conf, perplexity, filter_tag)), dpi = 300, format = 'pdf', bbox_inches='tight')
    #    plt.close(fig)
    #print '... done.'
